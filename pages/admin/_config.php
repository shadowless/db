<div class="cl-right">
<div class="s-bk-lf">
	<div class="hst-title">Настройки</div>
</div>
<div class="clr"></div>	
<?PHP
...
if(isset($_POST["admin"])){
	...
	if($items_per_coin < 1 OR $items_per_coin > 50000){
		$errors = false; echo "<center><font color = 'red'><b>Сколько продукции = 1 серебра, должно быть от 1 до 50000</b></font></center><BR />"; 
	}
	
	if($tomat_in_h < 6 OR $straw_in_h < 6 OR $pump_in_h < 6 OR $peas_in_h < 6 OR $pean_in_h < 6){
		$errors = false; echo "<center><font color = 'red'><b>Неверная настройка урожайности животных в час! Минимум 6</b></font></center><BR />"; 
	}
	
	
	if($amount_tomat_t < 1 OR $amount_straw_t < 1 OR $amount_pump_t < 1 OR $amount_peas_t < 1 OR $amount_pean_t < 1){
		$errors = false; echo "<center><font color = 'red'><b>Минимальная стоимость животного не должна быть менее 1го серебра</b></font></center><BR />"; 
	}
