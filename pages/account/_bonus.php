...
Бонус выдается сахаром на счет для покупок. <BR />
Сумма бонуса генерируется случайно от <b><?=$bonus_min;?></b> до <b><?=$bonus_max;?></b> сахара.
<BR /><BR />
<?PHP
$ddel = time() + 60*60*24;
$dadd = time();
...
$hide_form = false;
	...
	if($db->FetchRow() == 0){
		...
		if(isset($_POST["bonus"])){
			...
			echo "<center><font color = 'green'><b>На Ваш счет для покупок зачислен бонус в размере {$sum} сахара</b></font></center><BR />";
...
<table cellpadding='3' cellspacing='0' border='0' bordercolor='#336633' align='center' width="99%">
  ...
  $db->Query("SELECT * FROM db_bonus_list ORDER BY id DESC LIMIT 20");
	...
	if($db->NumRows() > 0){
		...
		?>
		<tr class="htt">
    		<td style="border: 1px dashed #db8;" align="center" ><?=$bon["id"]; ?></td>
    		<td style="border: 1px dashed #db8;" align="center" ><?=$bon["user"]; ?></td>
    		<td style="border: 1px dashed #db8;" align="center" ><?=$bon["sum"]; ?></td>
			<td style="border: 1px dashed #db8;" align="center" ><?=date("d.m.Y",$bon["date_add"]); ?></td>
  		</tr>
		<?PHP
